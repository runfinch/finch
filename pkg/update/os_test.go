...
func createMockBinary(_ *testing.T, dir string, exitCode int, targetOS string) string {
	var path string
	var content string

	if targetOS == "windows" {
		// On Windows, create a .exe file with batch content
		path = filepath.Join(dir, "finch.exe")
...
func TestApplyUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockBinPath := createMockBinary(t, binDir, 0, runtime.GOOS)
