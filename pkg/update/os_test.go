...
func (m *mockTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	case RootfsURL:
		...
		manifest := RootfsManifest{Filename: "test-rootfs.qcow2", SHA512Sum: testDigest}
		data, _ := json.Marshal(manifest)
		return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(string(data)))}, nil
	default:
...
func createMockBinary(_ *testing.T, dir string, exitCode int) string {
...
func TestCheckForUpdates(t *testing.T) {
	...
	tests := []struct {
		...
		{"windows amd64", "windows", "amd64", func(_ *mockTransport) {}, "test-rootfs.qcow2", false},
...
func TestVMOperations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockPath := createMockBinary(t, tempDir, tt.exitCode)
...
func TestConstructDownloadURL(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			baseURL:    "https://deps.runfinch.com",
			osBasename: "finch-rootfs-produxction-amd64-123456789.qcow2",
			expected:   "https://deps.runfinch.com/common/x86-64/finch-rootfs-produxction-amd64-123456789.qcow2",
		},
		{
			name:       "Another Windows rootfs artifact",
			baseURL:    "https://deps.runfinch.com",
			osBasename: "finch-rootfs-production-amd64-987654321.qcow2",
			expected:   "https://deps.runfinch.com/common/x86-64/finch-rootfs-production-amd64-987654321.qcow2",
		},
		{
...
func TestApplyUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		{"windows-amd64", "windows", "amd64", "test-rootfs.qcow2", "x86_64"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockBinPath := createMockBinary(t, binDir, 0)
