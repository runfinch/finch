...
func TestGetFinchVersions(t *testing.T) {
	...
	// just verify the function returns strings
...
func TestGetDownloadURL(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.goos == runtime.GOOS && tc.goarch == runtime.GOARCH {
				result, err := getDownloadURL(tc.version, tc.goos)
				if tc.expectError {
					assert.Error(t, err)
				} else {
	...
	t.Run("unsupported platform coverage", func(t *testing.T) {
		...
		result, err := getDownloadURL("1.0.0", runtime.GOOS)
		switch runtime.GOOS {
		case "linux":
			assert.Error(t, err)
	...
	t.Run("empty version", func(t *testing.T) {
		...
		result, err := getDownloadURL("", runtime.GOOS)
		switch runtime.GOOS {
		case "linux":
			assert.Error(t, err)
		case "darwin", "windows":
...
func TestInstallUpdate(t *testing.T) {
	...
	t.Run("unsupported platform error", func(t *testing.T) {
		...
		if runtime.GOOS != "darwin" && runtime.GOOS != "windows" {
			tempDir := t.TempDir()
			filePath := filepath.Join(tempDir, "test.pkg")

			err := os.WriteFile(filePath, []byte("test"), 0o644)
			assert.NoError(t, err)

			err = installUpdate(filePath)
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "unsupported platform")
		}
	})

	t.Run("function signature check", func(t *testing.T) {
		t.Parallel()

		// Just verify the function exists without calling it on supported platforms
		assert.NotNil(t, installUpdate)
	})
}
...
func TestDownloadAndInstallUpdateInternal(t *testing.T) {
	t.Parallel()

	t.Run("platform error path", func(t *testing.T) {
		t.Parallel()

		// Test platform-specific error without network calls
		if runtime.GOOS == "linux" {
			err := DownloadAndInstallUpdate("1.0.0")
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "failed to get download URL")
		} else {
			assert.NotNil(t, DownloadAndInstallUpdate)
		}
	})
}
