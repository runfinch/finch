...
func TestCosignVerifier(t *testing.T) {
	...
	t.Run("rejects tampered manifest", func(t *testing.T) {
		...
		tampered := bytes.Clone(manifestBytes)
		tampered[0] = 'X'
...
func TestExtractGHRunID(t *testing.T) {
	...
	t.Run("extracts run ID from filename with multiple extensions", func(t *testing.T) {
		t.Parallel()
		runID, err := extractGHRunID("finch-al2023-rootfs-x86-64-32273812529.tar.gz")
		require.NoError(t, err)
		assert.Equal(t, int64(32273812529), runID)
	})
