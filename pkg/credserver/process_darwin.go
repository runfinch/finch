...
// If logPath is provided, daemon logs will be written to that file.
func StartCredentialServer(finchRootPath string, logPath string) error {
	...
	if data, err := os.ReadFile(pidFile); err == nil {
		...
		if !isDaemonRunning(pidFile) {
			fmt.Fprintf(os.Stderr, "Warning: Stale PID file found (PID: %s), cleaning up\n", pid)
			_ = os.Remove(pidFile)
		}
	}
	...
	cmd := exec.Command(daemonPath, socketPath, pidFile)
	...
	if logPath != "" {
		...
		cmd.Args = append(cmd.Args, logPath)
	}
