...
func getFinchDir() string {
	// The logic here is similar to FinchRootDir() in pkg/path/finch_windows.go.
	var finchDir string
	if runtime.GOOS == "windows" {
		finchDir = os.Getenv("LOCALAPPDATA")
		gomega.Expect(finchDir).ShouldNot(gomega.BeEmpty())
	} else {
		home, err := os.UserHomeDir()
		gomega.Expect(err).ShouldNot(gomega.HaveOccurred())
		finchDir = home
	}
	return filepath.Join(finchDir, ".finch")
}
...
func startLocalDepsServer(tmpDir, newRunID string) *localDepsServer {
	...
	// Windows uses a .tar.gz rootfs artifact; macOS uses a .qcow2 disk image.
	imageExt := ".qcow2"
	if runtime.GOOS == "windows" {
		imageExt = ".tar.gz"
	}
	newImageName := fmt.Sprintf("finch-al2023-os-image-%s-%s%s", runtime.GOARCH, newRunID, imageExt)
...
func signManifest(tmpDir string, manifestBytes []byte) (bundleBytes []byte, issuer, identity string) {
	...
	// #nosec G304 -- path is from a controlled test helper, not user input
