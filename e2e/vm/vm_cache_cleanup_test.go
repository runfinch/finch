...
var testVMPrune = func(o *option.Option, installed bool) {
	ginkgo.Describe("Test System Prune Cleans Up Build Cache, Images, Containers", func() {
		ginkgo.It("check prune cleans up build cache, images and containers", func() {
			...
			log.L.Info("Waiting for some time for resources to be gc")
...
func verifyResourcesExist(o *option.Option, limaCtlO *option.Option) {
	...
	gomega.Expect(command.StdoutStr(o, "container", "ls", "-a", "-q")).ToNot(gomega.Equal(""),
		"Expected at least one container to exist")
...
func verifyResourcesCleaned(o *option.Option, limaCtlO *option.Option) {
	...
	snapshotOutput := command.Stdout(limaCtlO, "shell", "finch", "sudo", "ctr", "snapshot", "ls")
	snapshotOutputStr := strings.TrimSpace(string(snapshotOutput))
	lines := strings.SplitN(snapshotOutputStr, "\n", 2)
	if len(lines) > 1 {
		snapshotOutputStr = lines[1]
	} else {
		snapshotOutputStr = ""
	}
	gomega.Expect(snapshotOutputStr).To(gomega.Equal(""),
		"Expected snapshots to be cleaned up after prune")
}
