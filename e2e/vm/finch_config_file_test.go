...
var testFinchConfigFile = func(o *option.Option) {
	ginkgo.Describe("finch config file", func() {
		ginkgo.It("should store login credentials", func() {
			...
			// `finch run --entrypoint htpasswd public.ecr.aws/docker/library/httpd -Bbn user pwd`.
			...
			htpasswd := "user:$2y$05$eWFC9xitI6C7Fn7L9L8Gy.urZjJSjOVlvfkGIr2vPO5oNz2RpZNoa"
			...
			registry := fmt.Sprintf("localhost:6000")
			command.Run(o, "login", registry, "-u", "user", "-p", "pwd")
