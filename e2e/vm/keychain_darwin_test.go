...
var testNativeCredHelper = func(o *option.Option, installed bool) {
	ginkgo.Describe("Native Credential Helper", func() {
		...
		ginkgo.It("should work with plaintext config", func() {
			...
			ginkgo.By("Login with plaintext config shows Finch documentation URL")
			...
			stderrOutput := string(loginResult.Err.Contents())
			gomega.Expect(stderrOutput).To(gomega.ContainSubstring("https://runfinch.com/docs/container-images/authentication/"))
			gomega.Expect(stderrOutput).ToNot(gomega.ContainSubstring("https://docs.docker.com/go/credential-store/"))
		...
		ginkgo.It("should manage daemon lifecycle and prevent duplicate starts", func() {
			...
			ginkgo.By("Verifying daemon starts and PID file exists")
			...
			ginkgo.By("Verifying daemon process is running")
			...
			gomega.Expect(runningCount).To(gomega.Equal(1), "Should have exactly one credserver process running")
		})
		ginkgo.It("should support debug logging when FINCH_CRED_DEBUG is set", func() {
			cleanupKeychain := setupKeychain()
			ginkgo.DeferCleanup(cleanupKeychain)
			...
			_ = os.Setenv("FINCH_CRED_DEBUG", "1")
			ginkgo.DeferCleanup(func() { _ = os.Unsetenv("FINCH_CRED_DEBUG") })
			...
			gomega.Expect(string(logContent)).To(gomega.ContainSubstring("Credential server ready"))
		})
	})
}
