...
const (
	// guestSocketPath is the Unix socket path inside the VM for credential requests.
	// This must match the guestSocket path configured in mac.yaml portForwards.
	guestSocketPath = "/run/user/%d/finch-creds.sock"
)
	...
	if err != nil {
		...
		if _, statErr := os.Stat(socketPath); os.IsNotExist(statErr) {
			fmt.Fprintf(os.Stderr,
				"Warning: Credential server socket not found at %s. "+
					"The credential server may not be running. "+
					"Try restarting the VM with 'finch vm stop' and 'finch vm start'.\n",
				socketPath)
		}
...
func getSocketPath() string {
	...
	return fmt.Sprintf(guestSocketPath, uid)
}
