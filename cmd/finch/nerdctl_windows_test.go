...
var augmentedPath = filepath.Join(string(filepath.Separator), "mnt", "c", "workdir")
var wslPath = filepath.ToSlash(augmentedPath)
	...
	testCases := []struct {
	...
	}{
		{
			...
			mockSvc: func(
				...
				lcc.EXPECT().Create("shell", "--workdir", wslPath, limaInstanceName, "sudo", "-E", nerdctlCmdName, "image", "build", "-t", "demo", ".").Return(c)
				c.EXPECT().Run()
			},
		},
		{
