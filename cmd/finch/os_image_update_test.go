...
func TestOSImageUpdateAction_run(t *testing.T) {
	...
	t.Run("checkOnly writes metadata and returns", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(finchDir, 0o750))

		var serverURL string
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			switch r.URL.Path {
			case "/manifest.json":
				_, err := w.Write(newTestManifest(t, serverURL, "99999999999"))
				require.NoError(t, err)
			case "/manifest.json.bundle":
				_, err := w.Write([]byte("fake-bundle"))
				require.NoError(t, err)
			}
		}))
		defer server.Close()
		...
		require.NoError(
			...
			os.WriteFile(
				filepath.Join(
					osDir,
					fmt.Sprintf(
						"finch-image-%s-11111111111.qcow2",
						runtime.GOARCH,
					),
				),
				[]byte("img1"),
		...
		for _, runID := range updateRunIDs {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case "/manifest.json.bundle":
					...
					require.NoError(t, err)
				default:
					_, err := w.Write(imageContents[runID])
					require.NoError(t, err)
				}
			}))
			serverURL = server.URL + "/"
...
`, osDir, currentImage)
	...
	t.Run("no update available logs and returns", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			switch r.URL.Path {
			case "/manifest.json":
				_, err := w.Write(newTestManifest(t, serverURL, "11111111111"))
				require.NoError(t, err)
			case "/manifest.json.bundle":
				_, err := w.Write([]byte("fake-bundle"))
				require.NoError(t, err)
