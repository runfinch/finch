...
func TestPersistentPreRunE_UpdateNotification(t *testing.T) {
	...
	t.Run("does not show notification for support-bundle subcommands", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		l := mocks.NewLogger(ctrl)
		ecc := mocks.NewCommandCreator(ctrl)
		...
		parentCmd := &cobra.Command{Use: "support-bundle"}
		mockCmd := &cobra.Command{Use: "generate"}
		parentCmd.AddCommand(mockCmd)
		mockCmd.Flags().Bool("debug", false, "")
