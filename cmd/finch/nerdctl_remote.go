...
func (nc *nerdctlCommand) processArgs(args []string, envSet *orderedmap.OrderedMap) ([]string, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case strings.HasPrefix(arg, "--env-file"):
			nc.handleEnvFileArg(arg, args, i, envSet)
...
func (nc *nerdctlCommand) handleEnvFileArg(arg string, args []string, i int, envSet *orderedmap.OrderedMap) {
	var envFile string
	...
	if envFile != "" {
		unassignedVars := nc.processEnvFile(envFile, envSet)
...
func (nc *nerdctlCommand) processEnvFile(envFile string, existingVars *orderedmap.OrderedMap) []string {
	file, err := nc.fs.Open(filepath.Clean(envFile))
	if err != nil {
		nc.logger.Warnf("Error opening env file: %v", err)
		return []string{}
	}
	...
	if err := scanner.Err(); err != nil {
		nc.logger.Warnf("Error reading env file: %v", err)
		return []string{}
	}
